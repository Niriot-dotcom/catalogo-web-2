...
export const load: PageLoad = async ({ params }) => {
  ...
  const q = query(
    ...
    where("pageCategory", "==", "Accesorios de cocina"),
  );
  ...
  function movetoAnotherPosition(fromIndex: number, toIndex: number) {
    var element = products[fromIndex];
    products.splice(fromIndex, 1);
    products.splice(toIndex, 0, element);
  }
  movetoAnotherPosition(
    products.indexOf(
      products.filter((product) => product.pageTitle === "Sartén Fort")[0],
    ),
    1, // 0 is portadilla
  );
