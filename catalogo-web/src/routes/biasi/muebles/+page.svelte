<script lang="ts">
  ...
  import BiasiPuffs from "$lib/custom-pages/biasiPuffs.svelte";
  ...
  const puffPage = pages.filter((page) => page.pageTitle == "Puffs")[0];
...
{#each pages as page, i}
  ...
  {:else if page.pageTemplate == "videoAndGrid"}
    <VideoAndGrid
      ...
      bind:page
    />
  {:else if page.pageTemplate == "Puffs (invierno)"}
    <BiasiPuffs {puffPage} bind:selectedProduct />
  {/if}
{/each}
